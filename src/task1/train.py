...
def evaluate(
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(iter_):
            ...
            correct_num, all_num = utils.tag_accuracy(prediction, labels)
...
def train(
    ...
    model_output_path = os.path.join(model_base_path)
    if not os.path.exists(model_output_path):
        os.makedirs(model_output_path)

    params["load_model_path"] = None
    ...
    if (params['do_evaluate']):
        ...
        if params["final_evaluate"]:
            ...
            evaluate(model, valid_dataloader, device=device, logger=logger)
...
def main(params):
    ...
    if (params['do_evaluate']):
        ...
        valid_dataset = data.process_data(
            valid_samples,
            tokenizer,
            params,
        )
