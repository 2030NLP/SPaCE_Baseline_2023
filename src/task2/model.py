...
class Task2TriggerModel(nn.Module):
    def __init__(self, params, tag_num=4):
        super(Task2TriggerModel, self).__init__()
...
class Task2ElementModel(nn.Module):
    def __init__(self, params, tag_num=16):
        super(Task2ElementModel, self).__init__()
        self.bert_model = BertModel.from_pretrained(params['base_model'])
        bert_output_dim = self.bert_model.config.hidden_size
        self.dropout = nn.Dropout(0.1)
