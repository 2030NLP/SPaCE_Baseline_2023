...
from model import Task2ElementModel
import data
import utils


def load_model(params):
    return Task2ElementModel(params, (15+1)) # 15 types 
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(iter_):
            ...
            input_ids, token_type_ids, attention_mask, labels, fact_labels, time_labels, distance_labels = batch
                
            tag_predictions, fact_predictions = model.predict(
                input_ids, 
                token_type_ids, 
                attention_mask,
            )

            tag_predictions = tag_predictions.detach().cpu().numpy()
            fact_predictions = fact_predictions.detach().cpu().numpy()
            labels = labels.detach().cpu().numpy()
            fact_labels = fact_labels.detach().cpu().numpy()
            correct_num, all_num = utils.element_accuracy(tag_predictions, fact_predictions, labels, fact_labels)
...
def main(params):
    ...
    train_tensor_data = data.process_element_data(
        train_samples,
        tokenizer,
        params,
    )
    ...
    if (params['do_evaluate']):
        ...
        valid_tensor_data = data.process_element_data(
            valid_samples,
            tokenizer,
            params,
        )
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(iter_):
            batch = tuple(t.to(device) for t in batch)
            input_ids, token_type_ids, attention_mask, tag_labels, fact_labels, time_labels, distance_labels = batch
                
            tag_loss, fact_loss = model(
                input_ids, 
                token_type_ids, 
                attention_mask,
                tag_labels,
            ...
            loss = params['lambda']*tag_loss + (1-params['lambda'])*fact_loss
            ...
            if (step + 1) % (params["print_interval"] * grad_acc_steps) == 0:
                if (logger):
                    logger.info(
                        "Step %d - epoch %d average loss: %.4f; tag loss: %.4f; fact loss: %.4f" %(
                            step,
                            epoch_idx,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_path', type=str, default='./data/model/task2_element')
